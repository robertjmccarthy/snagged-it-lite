  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error) {
        ...
        debug.log('Payment status:', paymentIntent?.status || 'unknown');
        if (paymentIntent?.status === 'requires_action') {
          debug.log('Payment requires additional action');
          // The payment requires additional action, but this will be handled by redirect
        }
      }
    } catch (error: any) {
...
export default function StripePayment({ shareId, userId, onSuccess, onError }: PaymentFormProps) {
  ...
  useEffect(() => {
    ...
    async function initializePayment() {
      try {
        setIsLoading(true);
        debug.log('Initializing Stripe payment');
