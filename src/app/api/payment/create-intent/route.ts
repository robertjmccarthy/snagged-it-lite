...
export async function POST(request: Request) {
  try {
    ...
    debug.log('Creating payment intent for share', { shareId, userId });
    
    if (!shareId || !userId) {
      debug.error('Missing required parameters for payment intent');
    ...
    try {
      ...
      if (error || !data) {
        debug.error('Builder share record not found:', error || 'No data returned');
        return NextResponse.json(
      ...
      debug.log('Builder share record found', data);
    } catch (dbError) {
      debug.error('Error verifying builder share record:', dbError);
      return NextResponse.json(
    ...
    debug.log('Creating Stripe payment intent');
    ...
    debug.log('Payment intent created successfully', { id: paymentIntent.id });
