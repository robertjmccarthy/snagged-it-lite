      ...
      if (!loading) {
        ...
        try {
          ...
          if (progress && stepIndex > progress.current_step && !progress.is_complete && !(isAccessingFirstStep && isStartingOver)) {
            ...
            router.replace(`/checks/inside/${redirectStep}`);
            return;
          }
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (isLastStep) {
        ...
        router.push(`/checks/inside/${nextStep}`);
      }
    } catch (error) {
        ...
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="bg-white shadow-sm rounded-xl p-6 md:p-8 border border-gray-100">
            <header className="mb-8">
              ...
              {checklistItem && (
                <p className="text-gray-dark">Check this area carefully and note any issues you find.</p>
              )}
            </header>
              ...
              <div className="flex flex-col md:flex-row md:justify-between gap-3 w-full">
                <button
                  onClick={() => {
                    setIsNavigating(true);
                    setIsLoading(true);
                    router.push(`/checks/inside/${stepIndex}/snags/new`);
                  }}
                ...
                >
                  {stepIndex === totalSteps ? "Finish inside checks" : "Continue to next check"}
                </button>
              </div>
            </div>
