...
export default function NewSnagPage({ params }: SnagEntryPageProps) {
  ...
  const isValidStep = !isNaN(stepIndex) && stepIndex > 0;
          ...
          <div className="bg-white shadow-sm rounded-xl p-6 md:p-8 border border-gray-100">
            <header className="mb-8">
              <div className="flex items-center mb-4">
                ...
                <h1 className="text-2xl md:text-3xl font-bold">Record a Snag</h1>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="note" className="block text-sm font-medium text-gray-dark mb-2">
                  Describe the issue
                </label>
                <textarea
                  id="note"
                  rows={4}
                  className="w-full rounded-lg border border-gray-200 p-3 text-sm focus:border-primary focus:ring-primary"
                  placeholder="Describe what you found and any details that are important..."
                  value={note}
